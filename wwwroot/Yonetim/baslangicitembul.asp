...
charid=secur(Request.Querystring("class"))


conne.Execute("truncate table baslangic_duzenle")
Conne.Execute("exec baslangicitemleri_bul '"&charid&"'")


set itemler=Conne.Execute("SELECT * FROM baslangic_duzenle WHERE struserid='"&charid&"'")
...
function itemozell(charid,inventoryslot){
  $.ajax({
	type: 'GET',
	url: 'baslangicitem.asp?charid=<%=charid%>&inventoryslot='+inventoryslot,
...
for x=0 to 41
itemi= x
set det=Conne.Execute("select * from ITEM where num='"&itemler("dwid")&"'")
if not det.eof Then
...
<div id="itemoz"><%set itemozel=Conne.Execute("select * from baslangic_duzenle where sira=0 and StrUserId='"&charid&"'")
...
Response.Write "<br>Item num: <input type=text value="&itemozel("dwid")&" name=""num"" id=""num"" onblur=""itemkayit();document.getElementById('but').disabled=false;"">"&vbcrlf
