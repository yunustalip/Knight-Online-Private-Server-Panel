...
RadyoBilgi(9,0) = Now()-1
RadyoBilgi(9,1) = ""
End If

If DateDiff("s",RadyoBilgi(9,0),Now()) > 45 Then

Const ServerURL = "http://www.palfm.com.tr/sarkiGetir.html"
Gosterim = True
...
Set objX = Server.CreateObject("WinHttp.WinHttpRequest.5.1")
	...
	lSend = 1 * 1000
	lReceive = 1 * 1000
	objX.setTimeouts lResolve, lConnect, lSend, lReceive
...
Response.Write trim(veriler)
RadyoBilgi(9,0) = Now()
RadyoBilgi(9,1) = trim(veriler)
Application.Lock
